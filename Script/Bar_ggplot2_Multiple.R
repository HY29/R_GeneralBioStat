...
d$Nutrient <- factor(d$Nutrient, levels=c("Cont.","Mg def.","High Mg"))

#Bar graph by ggplot2_cowplotStyle
#setEPS()
#postscript(file ="OutputFig/Barplot_Mg_NL.eps",width=2.5, height=3.0)

pdf(file ="OutputFig/Barplot_Mg_NL.pdf",width=4, height=4)

ggplot(d, aes(y=d$Mg_NL, x=d$Nutrient, fill=d$Light))+ #Imput Data
  #guides(color=FALSE, fill=FALSE)+ #Label
  stat_summary(fun.y = mean, geom="bar", position="dodge", color="black", size=0.5)+ #Barplot
  geom_point(aes(fill=d$Light), shape=21, size=2, color="black",position=position_dodge(0.9))+ #Rawdata plot
  ...
  scale_x_discrete(limits=c("Cont.","Mg def.","High Mg"))+ #Setting xaxis label
